...
class GA:
    ...
    def __init__(self, gen, popsize, mr, init, end, maze):
        self.gen = gen                                           # Total number of generations.
        self.curgen = 0
        self.popsize = popsize
        self.mr = mr                                             # Mutation rate.
        self.init = init
        self.end = end
        self.bestp = pathfinding(init, end, maze.getMaze())      # Stores the best path found by pathfinding (A*)
        self.initPopulation(init, end, maze)


    # Initializes the population of the genetic algorithm with players.
    def initPopulation(self,init,end, maze):
        for i in range(self.popsize):
            self.population.append(Player(init, end, maze, self.bestp))
        self._fitness()                                               # A method call to evaluate the fitness of each player.
